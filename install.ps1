...
Done. Open a NEW terminal, then run:  thmes

Windows notes:
  * MLX is Apple-Silicon only -> use an Ollama model:  set THMES_MODEL=ol:llama3.2:3b
    (install Ollama from https://ollama.com, then: ollama pull llama3.2:3b)
  * The bash tool, gemma, mlx-serve-* and the web terminal (thmes-web) are Unix-only.
  * For full features (web terminal, bash tool) run thmes under WSL:
        wsl --install   then inside WSL:  ./install.sh
"@
